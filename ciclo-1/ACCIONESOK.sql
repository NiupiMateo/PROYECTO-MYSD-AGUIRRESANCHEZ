...
/* ACCION 4: Equipo -> Escuela (CASCADE)
Pero primero borraramos Inscripcion (que referencia Escuela directamente) para evitar error de FK 
TEST 4: DELETE Escuela borra Equipo en CASCADE (despues de borrar Inscripcion)*/
...
/* ACCION 5: Verificar que Pago se elimina por CASCADE 
TEST 5: DELETE Pago confirma que la relacion existe*/
SELECT COUNT(*) AS "Pagos antes" FROM Pago WHERE idPago = 400;
DELETE FROM Pago WHERE idPago = 400;
SELECT COUNT(*) AS "Pagos despues" FROM Pago WHERE idPago = 400;
ROLLBACK;
